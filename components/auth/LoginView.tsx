        ...
        <div className="w-full max-w-md">
          ...
          {showFallbackAuth ? (
            <div className="space-y-4">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  setIsLoading(true);
                  try {
                    const { user, error: authError } = createUserFromCredentials(email, password);
                    if (authError) {
                      setError(authError);
                      setIsLoading(false);
                    } else if (user) {
                      // Store auth in localStorage for dummy auth system
                      localStorage.setItem("__demo_auth__", JSON.stringify(user));
                      // Reload page to trigger app controller to pick up auth
                ...
                disabled={isLoading}
                className="w-full bg-blue-600 text-white font-semibold py-3 rounded-lg hover:bg-blue-700 transition-all disabled:bg-gray-400"
              >
            ...
            <>
              ...
              <button
                type="button"
                onClick={() => setShowFallbackAuth(true)}
                className="w-full mt-4 text-slate-600 text-sm hover:text-slate-900 py-2"
              >
